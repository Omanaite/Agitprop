...
export function StudioGalleryManager() {
  ...
  async function load() {
    ...
    setErrors([]);
    setIsLoading(true);
    const res = await fetch("/api/studio/gallery-items");
    const galleriesRes = await fetch("/api/studio/galleries");
    if (!res.ok) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const res = await fetch(
      isEdit ? `/api/studio/gallery-items/${form.id}` : "/api/studio/gallery-items",
      {
  ...
  async function handleDelete(id: string) {
    ...
    setStatus("");
    setIsSaving(true);
    const res = await fetch(`/api/studio/gallery-items/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const data = await res.json().catch(() => null);
      setErrors(data?.errors ?? []);
  ...
  async function createFromUploads() {
    ...
    const requests = bulkUploads.map((url, index) =>
      fetch("/api/studio/gallery-items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function saveOrder() {
    ...
    const updates = items.map((item, index) =>
      fetch(`/api/studio/gallery-items/${item.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: item.title,
