...
function ProviderIcon({ provider }: { provider: string }) {
  if (provider === "google") return <FaGoogle aria-hidden className="size-4" />;
  if (provider === "github") return <FaGithub aria-hidden className="size-4" />;
  return <span className="size-4" />;
}

export function StudioIntegrationsManager() {
  ...
  async function updateIntegration(
    provider: string,
    statusValue: Integration["status"]
  ) {
    setStatus("");
    setErrors([]);
    setIsSaving(true);
  ...
  function connectProvider(provider: string) {
    ...
    window.location.href = `/api/auth/oauth?provider=${provider}&next=${next}`;
  }
  ...
  return (
    <section className="admin-card p-6 md:p-7">
      ...
      <p className="admin-muted mt-2 mb-4 text-sm leading-6">
        Connect OAuth and cloud providers for upload readiness.
      </p>
          ...
          return (
            <div key={provider} className="admin-card-soft p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-[var(--admin-muted)]">
                  <ProviderIcon provider={provider} />
                  {provider}
                </div>
                <span className="admin-chip">{current?.status ?? "disconnected"}</span>
              </div>
