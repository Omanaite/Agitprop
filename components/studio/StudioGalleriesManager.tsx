...
export function StudioGalleriesManager() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const res = await fetch(
      isEdit ? `/api/studio/galleries/${form.id}` : "/api/studio/galleries",
      {
  ...
  async function handleDelete(id: string) {
    ...
    setStatus("");
    setIsSaving(true);
    const res = await fetch(`/api/studio/galleries/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const data = await res.json().catch(() => null);
      setErrors(data?.errors ?? []);
